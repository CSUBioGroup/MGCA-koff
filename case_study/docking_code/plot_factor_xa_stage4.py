...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stage2-root",
        type=Path,
        default=repository_root / "results" / "case_study" / "factor_xa_stage2",
    )
    parser.add_argument(
        "--stage3-root",
        type=Path,
        default=repository_root / "results" / "case_study" / "factor_xa_stage3_docking",
    )
...
def benchmark_label(model: str) -> str:
    """Return a publication-facing benchmark label for an internal model key."""
    return "Zhao et al. dataset" if model == "2773" else f"{model} dataset"
...
def plot_reference_style_figure(
    ...
    fig.text(
        0.52, 0.018,
        f"{benchmark_label(model)} five-checkpoint mean; 0–1 robust normalization of ΔpKoff. "
...
def plot_sequence_panel(
    ...
    ax.axis("off")
    ax.set_title(
        f"B  Factor Xa sequence: {benchmark_label(model)} five-checkpoint mean occlusion sensitivity",
...
def combine_panels(
    ...
    fig.suptitle(
        f"Factor Xa post-hoc occlusion interpretation ({benchmark_label(model)})",
