...
import ESM_Morgan_Hybrid_Fusion_cgrs_v10 as mgca
from utils import (
    atomic_npz,
    atomic_json,
    atomic_text,
...
def parse_args():
    ...
    parser.add_argument("--window-layout", default="legacy_anchors_v1")
    ...
    parser.add_argument("--gate-prior-weight", type=float, default=0.001)
    parser.add_argument("--branch-margin", type=float, default=0.01)
    parser.add_argument("--drug-gate-init", type=float, default=0.10)
    parser.add_argument("--drug-gate-cap", type=float, default=0.30)
    parser.add_argument("--joint-gate-init", type=float, default=0.03)
    parser.add_argument("--joint-gate-cap", type=float, default=0.12)
...
def main():
    ...
    if min(args.drug_utility_weight, args.joint_utility_weight, args.gate_prior_weight, args.branch_margin) < 0:
        raise ValueError("utility/prior weights and branch margin must be non-negative")
    if not 0 < args.drug_gate_init < args.drug_gate_cap <= 1:
        raise ValueError("require 0 < drug-gate-init < drug-gate-cap <= 1")
    if not 0 < args.joint_gate_init < args.joint_gate_cap <= 1:
        raise ValueError("require 0 < joint-gate-init < joint-gate-cap <= 1")
    ...
    identity = {
        ...
        "hyperparameters": {
            ...
            "drug_gate_cap": args.drug_gate_cap,
            "joint_gate_init": args.joint_gate_init,
            "joint_gate_cap": args.joint_gate_cap,
            ...
            "numerical_stability_revision": "cgrs_global_shrinkage_v1",
        },
        ...
        "source_v4_sha256": sha256_file(mgca.SOURCE_V4_SCRIPT),
        "source_v5_sha256": sha256_file(mgca.SOURCE_V5_SCRIPT),
        "source_v6_sha256": sha256_file(mgca.SOURCE_V6_SCRIPT),
        "source_v7_sha256": sha256_file(mgca.SOURCE_V7_SCRIPT),
        "source_v8_sha256": sha256_file(mgca.SOURCE_V8_SCRIPT),
        "source_v9_sha256": sha256_file(mgca.SOURCE_V9_SCRIPT),
