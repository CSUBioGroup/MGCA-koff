...
class ResumeIntegration(unittest.TestCase):
    def test_crash_at_early_stop_does_not_add_epoch_and_keeps_best(self):
        ...
        mgca.__file__=str(model_path);mgca.MODEL_VARIANT='synthetic_fixture_actual_unbounded';mgca.MAX_GRAD_NORM=5.
        mgca.LEGACY_SCRIPT=HERE/'runtime/local/ESM_Morgan_Hybrid_Fusion.py'
        mgca.SOURCE_V10_SCRIPT=HERE/'sources/original_v10_model.py'
        ...
        ns=dict(argparse=argparse,contextlib=contextlib,hashlib=hashlib,json=json,math=math,os=os,Path=Path,
                ...
                SCRIPT_DIR=RUNTIME,__file__=str(RUNTIME/'trainer.py'))
        ...
        source=ast.parse((RUNTIME/'trainer.py').read_text(encoding='utf-8-sig'))
        ...
        extract(RUNTIME/'trainer.py',[n.name for n in source.body if isinstance(n,ast.FunctionDef)],ns)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                for crash_epoch in (7,12):
                    ...
                    def crash_save(torchmodule,path,payload):
                        ...
                        if Path(path).name=='last_state.pt':
                            ...
                            self.assertEqual(payload['optimizer_state_dict']['param_groups'][1]['weight_decay'],0.)
                            self.assertEqual(payload['optimizer_state_dict']['param_groups'][0]['weight_decay'],.0001)
                ...
                # Exercise actual formal export, prediction/archive seal, and corruption refusal.
                from worker import seal_run
                from workflow import verified
                atomic_text(root/'test.csv','FASTA,SMILES,pkoff\nAAAA,CC,0.5\nBBBB,CO,1.5\n')
                def formal_prepare(args,device):
                    parts=('train','val','test')
                    rows=[legacy['read_labeled_rows'](str(root/(p+'.csv'))) for p in parts]
                    return [dataset]*3,rows,dict(esm2=cache,morgan=cache,esm2_hit_before=True,morgan_hit_before=True)
                ns['prepare_data']=formal_prepare
                formal=root/'formal';sys.argv=argv(formal,'true')
                sys.argv.remove('--selection-only');sys.argv+=['--test-csv',str(root/'test.csv')]
                ns['main']()
                task=dict(output_dir=str(formal),task_id='unfrozen',selection_only=False,
                    train_csv=str(root/'train.csv'),val_csv=str(root/'val.csv'),test_csv=str(root/'test.csv'))
                plan=dict(scientific={'settings':{'protein_warmup_epochs':5,'fixed_shrinkage_epochs':5}})
                seal_run(plan,task,types.SimpleNamespace(**ns))
                self.assertTrue(verified(task)['test_accessed'])
                self.assertTrue((formal/'cgrs_outputs.npz').is_file())
                atomic_text(formal/'metrics.json',(formal/'metrics.json').read_text(encoding='utf-8')+' ')
                with self.assertRaises(RuntimeError):verified(task)
        finally:
